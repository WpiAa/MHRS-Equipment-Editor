...
<!-- 装饰品 -->
    <div style="margin-bottom:10px;margin-left:20px;">
        ...
        <div>
            ...
            <el-button size="small" :icon="Search" round @click="showDecorations(0,armorDecorationsNow[0])">
                {{ deco[0].decoName }} 
            </el-button>
        </div>
        <div>
            <span style="font-family:Hiragino Sans GB;padding-top: 20px">Lv{{ armorDecorationsNow[1] }}:&nbsp;&nbsp;&nbsp;</span>
            <el-button size="small" :icon="Search" round @click="showDecorations(1,armorDecorationsNow[1])">
                {{ deco[1].decoName }} 
            </el-button>
        </div>
        <div>
            <span style="font-family:Hiragino Sans GB;padding-top: 20px">Lv{{ armorDecorationsNow[2] }}:&nbsp;&nbsp;&nbsp;</span>
            <el-button size="small" :icon="Search" round @click="showDecorations(2,armorDecorationsNow[2])">
                {{ deco[2].decoName }} 
            </el-button>
        </div>
    </div>
...
export default {
   ...
   mounted() {
        this.axios.get('/mhrice.json').then(res => {     // 获取public下的test.json文件数据
            ...
            this.divDecoraton(1,{ decoId:0 , decoName: "" });
            this.divDecoraton(2,{ decoId:0 , decoName: "" });
            this.divDecoraton(3,{ decoId:0 , decoName: "" });
            this.divDecoraton(4,{ decoId:0 , decoName: "" });
            ...
            this.costSkillId.forEach((e,index) => {
                e.skillId.forEach(element => {
                    this.options[0].children[index].children.push({   value: element,//二层值,id
   ...
   data() {
       return {
        ...
        //    numArmor_head_name_msg_mr: 0,
           ...
           lv3DecoList:[],
           lv4DecoList:[],
           deco: [ { decoId:0 , decoName: "" }, { decoId:0 , decoName: "" }, { decoId:0 , decoName: "" } ],
           decoAllSkill: [],//选择的饰品技能
   ...
   watch:{
        ...
        decoFlag(newValue ,oldValue){ //选择孔位时展示不同等级装饰品
            ...
            switch (parseInt(this.slotLv)) {
                ...
                default: console.log("slot level < 1 !");
                    break;
            }
        ...
        //     skillId: this.skillList[item.skill_id_list[0].Skill]+1, skillLv: this.skillList[item.skill_lv_list[0]] };
            ...
            if (this.skillComputeList.length>=1) { //数量大于等于1个，技能整合
            ...
            // for (let index = 0; index < this.skillComputeList.length; index++) { //清空怪异化后技能等级为0或以下的
            //     if ( parseInt(this.skillComputeList[index].level)<1 ){
            //         this.skillComputeList.splice(index,1);
            //         --index;
            //     }
            // }
        },
