...
export default {
   ...
   mounted() {
        this.axios.get('/mhrice.json').then(res => {     // 获取public下的test.json文件数据
            //大师头盔名字列表
            this.armor_head_name_msg_mr = res.data.armor_waist_name_msg_mr;
            ...
            res.data.armor.param.forEach(element => {
                if( 'Waist' in element.pl_armor_id ){
   ...
   methods: {
       chooseHead(index){
           ...
           this.headDetail.forEach(element => {
               //找出对应头盔并记录
               if( element.pl_armor_id.Waist == parseInt(this.headId) ){
                   //技能 技能等级
   ...
   watch:{
        ...
        //     skillId: this.skillList[item.skill_id_list[0].Skill]+1, skillLv: this.skillList[item.skill_lv_list[0]] };
            ...
            this.$emit( 'waistSkill' ,this.skillComputeList) ;
